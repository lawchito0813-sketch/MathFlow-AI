...
![項目首頁與功能入口](doc/pic/出題工作台展示1.png)

*圖 1：系統首頁提供多個數學教學場景入口，包括解題、批改、整卷處理與 DSE 出卷。*

![系統功能頁面示意](doc/pic/出卷工作台展示2.png)
...
![流程進度與結果持續更新](doc/pic/數學運算.png)

*圖 3：系統會持續回傳處理進度與階段結果，便於老師掌握當前狀態。*

![多步驟處理介面示意](doc/pic/出題工作台題目設置.png)
...
![整卷 PDF 批改頁面總覽](doc/pic/整卷批改頁面展示.png)
...
![DSE 出卷工作台主介面](doc/pic/出卷工作台展示2.png)
...
更具體地說，系統不是把一次回答直接當作最終結論，而是先讓 Solver A 與 Solver B 以兩條相對獨立的方式同時處理同一題，再由 Judge 對兩邊的關鍵步驟、推理方向、計算結果與最終答案做交叉比對。當兩邊結果一致時，最終輸出的可信度會明顯提高；當兩邊出現差異時，Judge 會進一步識別哪一邊的步驟更完整、哪一個計算更合理，從而降低單次失誤直接進入最終答案的機會。它的目標不是宣稱數學結果絕對不會出錯，而是透過「雙重驗算 + 最終判定」的方式，把解題準確度盡量提高到接近老師實際教學可接受的穩定水準。
...
![困難版解題中的雙路分析](doc/pic/數學運算.png)

*圖 9：困難版解題會同時展開兩條分析路徑，再由 Judge 作綜合判斷。*

![困難版結果與講解決策示意](doc/pic/單題批改題目反饋講解.png)
...
![整卷處理中的逐題追蹤](doc/pic/畫圖功能:出圖展示.png)

*圖 11：系統不僅能處理文本結果，也能在相關流程中結合圖形與可視化內容進行展示。*
...
![整卷結果匯總與觀察介面](doc/pic/ai整卷批改報告.png)
...
![DSE 工作台中的對話與草稿區](doc/pic/出題工作台展示1.png)

*圖 13：DSE 工作台支援對話區與草稿區並行使用，方便老師邊看邊改。*

![DSE 工作台中的活動與過程面板](doc/pic/出題工作台題目設置.png)
