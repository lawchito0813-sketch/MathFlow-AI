...
function renderShape(shape) {
  ...
  if (shape.type === 'polygon' && Array.isArray(shape.points)) {
    const points = shape.points.map(point => `${point.x || 0},${point.y || 0}`).join(' ')
...
function buildSvg(plan) {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="800" height="500" viewBox="0 0 800 500">
    ...
    <text x="24" y="32" font-size="20" fill="#111827">${escapeXml(plan.title || '數學題圖形')}</text>
...
export function renderDiagramSvgDataUrl(plan) {
