...
const TIMELINE_EVENTS = new Set([
  ...
  'review_started', 'review_score_plan_started', 'review_score_plan_done', 'review_done',
  'review_score_breakdown_ready', 'final_answer_ready', 'diagram_started', 'diagram_llm_started',
  'diagram_attempt_failed', 'diagram_retrying', 'diagram_done', 'diagram_error',
  'final_explanation_started', 'final_explanation_done', 'review_followup_started',
  'review_followup_done', 'stage_repairing', 'stage_compact_retry', 'stage_failed', 'session_error'
])
...
const TIMELINE_LABELS = {
  ...
  review_score_plan_started: '分數規劃開始', review_score_plan_done: '分數規劃完成', review_done: '批改完成',
  review_score_breakdown_ready: '最終得分已產生', final_answer_ready: '標準答案已產生',
  diagram_started: '開始生成圖形', diagram_llm_started: '畫圖 AI 開始',
  diagram_attempt_failed: 'Python 執行失敗', diagram_retrying: '準備重試作圖',
  diagram_done: '圖形生成完成', diagram_error: '圖形生成失敗',
  final_explanation_started: '批改後講解開始', final_explanation_done: '批改後講解完成',
  review_followup_started: '追問開始', review_followup_done: '追問完成',
  stage_repairing: '模型重發中', stage_compact_retry: '精簡重發中',
  stage_failed: '階段失敗', session_error: '流程錯誤'
}
...
function formatScorePlan(payload) {
  return [
    `總分：${payload.totalMarks} 分`,
    `來源：${payload.totalMarksSource === 'problem' ? '題目標示' : '模型估算'}`,
    Array.isArray(payload.subparts) && payload.subparts.length > 0 ? `分題：\n- ${payload.subparts.map(item => `${item.label} ${item.maxMarks} 分`).join('\n- ')}` : '',
    payload.reasoning ? `說明：${payload.reasoning}` : ''
  ].filter(Boolean).join('\n\n')
}

function formatScoreBreakdown(payload) {
  return [
    `總得分：${payload.awardedTotalMarks}/${payload.maxTotalMarks}`,
    Array.isArray(payload.scoreBreakdown) && payload.scoreBreakdown.length > 0
      ? payload.scoreBreakdown.map(item => `${item.label}) ${item.awardedMarks}/${item.maxMarks}${item.comment ? `｜${item.comment}` : ''}`).join('\n')
      : ''
  ].filter(Boolean).join('\n\n')
}
...
function resetView() {
  ...
  timeline.innerHTML = ''; reviewStream.textContent = ''; reviewResult.textContent = ''; reviewSummary.textContent = ''; referenceAnswer.textContent = ''
...
function renderReviewSummary(payload) {
  reviewSummary.textContent = [`答案判定：${payload.answerVerdict || ''}`, `方法判定：${payload.methodVerdict || ''}`, payload.whyWrong ? `錯因：${payload.whyWrong}` : '', payload.suggestedNextStep ? `建議下一步：${payload.suggestedNextStep}` : ''].filter(Boolean).join('\n\n')
  referenceAnswer.textContent = [payload.referenceAnswer ? `標準答案：${payload.referenceAnswer}` : '', payload.referenceReasoning ? `標準理由：${payload.referenceReasoning}` : '', payload.scoreJudgement ? `評分說明：${payload.scoreJudgement}` : ''].filter(Boolean).join('\n\n')
...
function connectEvents(sessionId) {
  ...
  return new Promise(resolve => {
    ...
    bind('review_delta', payload => { reviewStream.textContent += payload.delta || '' })
