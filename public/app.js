...
const solverAOutput = document.querySelector('#solver-a')
const solverBOutput = document.querySelector('#solver-b')
const judgeOutput = document.querySelector('#judge')
const solverAStreamOutput = document.querySelector('#solver-a-stream')
const solverBStreamOutput = document.querySelector('#solver-b-stream')
...
const TIMELINE_LABELS = {
  ...
  solver_a_started: 'Solver A 開始',
  solver_b_started: 'Solver B 開始',
  solver_a_done: 'Solver A 完成',
  solver_b_done: 'Solver B 完成',
...
function setFormattedText(element, text) {
  element.textContent = normalizeDisplayText(text)
}
...
function appendTimeline(title, payload) {
  ...
  if (title === 'session_created' || title === 'events_connected') {
    ...
    detail = [payload?.finalAnswer || '', payload?.providerLabel ? `(${payload.providerLabel})` : ''].filter(Boolean).join(' ')
  } else if (title === 'final_explanation_done') {
...
function resetView() {
  ...
  timeline.innerHTML = ''
  solverAOutput.textContent = ''
  solverBOutput.textContent = ''
  judgeOutput.textContent = ''
  solverAStreamOutput.textContent = ''
  solverBStreamOutput.textContent = ''
...
function connectEvents(sessionId) {
  ...
  return new Promise(resolve => {
    ...
    bind('input_received', () => {})
    bind('solver_a_started', () => {})
    bind('solver_b_started', () => {})
    bind('judge_started', () => {})
    ...
    bind('solver_a_delta', payload => {
      solverAStreamOutput.textContent += payload.delta || ''
    })

    bind('solver_b_delta', payload => {
      solverBStreamOutput.textContent += payload.delta || ''
    })

    bind('solver_a_done', payload => {
      solverAOutput.textContent = JSON.stringify(payload, null, 2)
    })

    bind('solver_b_done', payload => {
      solverBOutput.textContent = JSON.stringify(payload, null, 2)
    })
